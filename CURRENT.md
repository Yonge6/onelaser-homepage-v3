...
- The previous-generation disclosure is nested at the bottom of the `CURRENT LINEUP` catalog surface, after the current product grid and before the conditional comparison module.
- The previous-generation disclosure keeps its rounded frame but uses a white inset surface against the catalog's `#F5F5F7` background, restoring clear section separation while its header, intro and product grid share one internal left/right alignment.
